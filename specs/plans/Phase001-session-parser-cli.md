...
Claude Code 세션 JSONL 파일을 파싱하여 Go 구조체로 변환한다. CLI 뼈대를 잡고 `whylog sessions` 명령으로 세션 목록을 출력하는 것까지.
...
- [ ] `cmd/whylog/` — main.go (cobra CLI)
- [ ] `internal/model/` — 데이터 모델
- [ ] `internal/parser/` — JSONL 파서
- [ ] `go mod init github.com/clari/whylog`
...
### Step 5. CLI — `whylog sessions`

- [ ] `cmd/whylog/main.go` — cobra 기반
  - `whylog sessions` — 현재 프로젝트의 세션 목록 (ID, 시간, 첫 사용자 메시지)
  - `--sessions-dir` 옵션

### Step 6. 검증

- [ ] whylog 프로젝트 자체 세션 JSONL로 `whylog sessions` 동작 확인
